...
def _wrap_proxy_error(err: Exception, proxy_scheme: str | None) -> ProxyError:
    ...
    is_likely_http_proxy = (
        "wrong version number" in error_normalized or "unknown protocol" in error_normalized or "record layer failure" in error_normalized
    )
    http_proxy_warning = (
    ...
    new_err = ProxyError(
        f"Unable to connect to proxy{http_proxy_warning if is_likely_http_proxy and proxy_scheme == 'https' else ''}",
        err,
    )
