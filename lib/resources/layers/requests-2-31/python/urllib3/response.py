...
class MultiDecoder(ContentDecoder):
    ...
    def __init__(self, modes: str) -> None:
        ...
        if len(encodings) > self.max_decode_links:
            raise DecodeError(f"Too many content encodings in the chain: {len(encodings)} > {self.max_decode_links}")
...
class HTTPResponse(BaseHTTPResponse):
    ...
    def read1(
        ...
        if self._has_decoded_content:
            if not decode_content:
                raise RuntimeError("Calling read1(decode_content=False) is not supported after read1(decode_content=True) was called.")
    ...
    def stream(self, amt: int | None = 2**16, decode_content: bool | None = None) -> typing.Generator[bytes]:
        ...
        if self.chunked and self.supports_chunked_reads():
            ...
            while not is_fp_closed(self._fp) or len(self._decoded_buffer) > 0 or (self._decoder and self._decoder.has_unconsumed_tail):
